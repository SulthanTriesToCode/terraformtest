...
ansible-playbook ansible/db-playbook.yml -i infra/ansible-inventory.yml --private-key "${GITHUB_WORKSPACE}/infra/my_A2_key" -o StrictHostKeyChecking=no
#run app playbook yml
echo "Running ansible to configure App"
ansible-playbook ansible/app-playbook.yml -e "db_public_hostname=${db_public_hostname}" -i infra/ansible-inventory.yml --private-key "infra/my_A2_key" -o StrictHostKeyChecking=no
#run app-clone playbook yml
echo "Running ansible to configure App Clone"
ansible-playbook ansible/app-clone-playbook.yml -e "db_public_hostname=${db_public_hostname}" -i infra/ansible-inventory.yml --private-key "infra/my_A2_key" -o StrictHostKeyChecking=no